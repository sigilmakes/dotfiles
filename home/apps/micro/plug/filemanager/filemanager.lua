...
-- Min/max width for the tree pane
local tree_min_width = 30
local tree_max_width = 60

-- Resize the tree pane, clamped to [tree_min_width, tree_max_width]
local function resize_tree(width)
	if width < tree_min_width then
		width = tree_min_width
	elseif width > tree_max_width then
		width = tree_max_width
	end
	tree_view:ResizePane(width)
end
...
local function refresh_view()
	...
	-- If it's less than min width, resize back up
	if tree_view:GetView().Width < tree_min_width then
		resize_tree(tree_min_width)
	end
...
local function compress_target(y, delete_y)
	...
	if tree_view:GetView().Width > (tree_min_width + highest_visible_indent) then
		-- Shave off some width
        resize_tree(tree_min_width + highest_visible_indent)
	end
...
local function try_open_at_y(y)
	...
	elseif y > 2 and not scanlist_is_empty() then
		...
		if scanlist[y].dirmsg ~= "" then
			...
			-- If it's a file, then open it in the existing editor pane
			micro.InfoBar():Message("Filemanager opened ", scanlist[y].abspath)
			local buf = buffer.NewBufferFromFile(scanlist[y].abspath)
			-- Switch focus to the editor pane (next split from tree)
			tree_view:NextSplit()
			-- Replace the buffer in that pane instead of creating a new split
	...
	if scanlist[y].dirmsg == "+" then
		...
		if scan_results ~= nil then
			if scanlist[y].indent > highest_visible_indent and #scan_results >= 1 then
				...
				resize_tree(tree_view:GetView().Width + scanlist[y].indent)
			end
		end
...
local function open_tree()
	...
	-- Set the initial width of tree_view
    resize_tree(tree_min_width)
	...
	-- Use current_dir (which remembers last location within the session)
	-- On first open, current_dir is initialized to os.Getwd() at plugin load
